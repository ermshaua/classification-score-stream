...
if __name__ == '__main__':
    ...
    print(f"{name}: True Change Points: {cps}, Found Change Points: {found_cps}, "
          f"Scores: {np.round(scores, 3)}, Runtime: {np.round(runtimes.sum(), 3)} "
          f"Covering-Score: {np.round(covering_score, 3)}")